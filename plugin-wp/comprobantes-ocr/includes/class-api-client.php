...
class COCR_API_Client {
    ...
    public function upload_slip( string $file_path, string $api_url, string $api_key, int $timeout = 30 ): array|\WP_Error {
        ...
        if ( is_array( $result ) && ( $result['_cocr_status'] ?? '' ) === 'duplicado' ) {
            return $this->enrichDuplicateResult( $result, $api_url, $api_key );
        }
    ...
    /**
     * Enrich a 409 duplicate response with original comprobante fields.
     *
     * @param array  $result  Raw 409 detail from _parse_response.
     * @param string $api_url Base URL.
     * @param string $api_key API key.
     * @return array
     */
    private function enrichDuplicateResult( array $result, string $api_url, string $api_key ): array {
        $id_comprobante = $result['id_comprobante'] ?? null;
        $original = null;
        if ( $id_comprobante ) {
            $original = $this->fetchComprobante( $id_comprobante, $api_url, $api_key );
        }
        $campos = [];
        if ( is_array( $original ) && isset( $original['campos_extraidos'] ) ) {
            $campos = $original['campos_extraidos'];
        }
        return [
     ...
     *
     * @param string $idComprobante UUID of the original comprobante.
     * @param string $apiUrl        Base URL of the FastAPI service.
     * @param string $apiKey        API key.
     * @return array|null
     */
    private function fetchComprobante( string $idComprobante, string $apiUrl, string $apiKey ): ?array {
        $response = wp_remote_get(
            trailingslashit( $apiUrl ) . 'comprobante/' . rawurlencode( $idComprobante ),
            [
                'timeout' => 5,
    ...
    /**
     * Extract detail payload for 409 duplicate responses.
     *
     * @param string $body Raw JSON body.
     * @return array
     */
    private function extractConflictDetail( string $body ): array {
    ...
    private function _parse_response( array|\WP_Error $response ): array|\WP_Error {
        ...
        if ( is_wp_error( $response ) ) {
            ...
            if ( $code >= 500 ) {
                ...
                $result = $this->extractConflictDetail( $body );
            } elseif ( $code >= 400 ) {
                $result = new \WP_Error(
                    'cocr_client_error',
                    sprintf(
