...
- [x] **1.8.3** Coverage: `pytest --cov=api --cov-report=term --cov-fail-under=70`
  - **Resultado:** Coverage configurado en `pyproject.toml` con `[tool.coverage.run]` (`source=["."]`, `core="sysmon"`, omits `tests/`, `alembic/`, `models/seed.py`) y `[tool.coverage.report]` (`fail_under=70`, `exclude_also` para `pragma: no cover`/`TYPE_CHECKING`/etc). Comando: `cd api && uv run pytest tests/ --cov=.`. Resultados: **96% global**, todos los `services/` ≥ 90% (cache 100%, image 97%, ocr 96%, parser 95%, storage 90%). Routers: history 100%, upload 88%, health 94%. **Gotcha técnico** (gotcha 2026-05-09 nuevo): el tracer clásico de coverage 7.x pierde líneas dentro de funciones async ejecutadas vía `httpx.ASGITransport` — `routers/upload.py` reportaba 58% cuando los asserts de DB demostraban que TODO el cuerpo se ejecutaba. Solución: `core = "sysmon"` (basado en `sys.monitoring` de Python 3.12+) que mide async correctamente y bumpa la métrica a la cifra real (88%). Sin sysmon el threshold 70% global apenas pasaría con margen falso. Threshold global 70% como piso de regresión (no objetivo). Sanity check con `--cov-fail-under=99` confirma que el gate corta la suite correctamente.
