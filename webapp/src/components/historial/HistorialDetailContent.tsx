...
export function estadoToBadgeVariant(estado: WebComprobanteItem["estado_actual"]): BadgeProps["variant"] {
  switch (estado) {
    case "valido":
      return "valido";
    case "duplicado":
...
export function estadoLabel(estado: WebComprobanteItem["estado_actual"]): string {
...
export function estadoIcon(estado: WebComprobanteItem["estado_actual"]): string {
  switch (estado) {
    case "valido":
      return "check_circle";
    case "duplicado":
...
export function estadoIconColor(estado: WebComprobanteItem["estado_actual"]): string {
  switch (estado) {
    case "valido":
...
export function SimilarityGauge({ value }: Readonly<{ value: number }>) {
...
export interface HistorialDetailContentProps {
...
export function HistorialDetailContent({
  item,
  error,
  status403 = false,
  loading = false,
  similitud = 0,
}: Readonly<HistorialDetailContentProps>) {
  if (loading) {
    return (
      <div data-testid="historial-detail-loading" className="space-y-6">
  ...
  if (status403) {
    return (
      <div className="flex flex-col gap-4">
        ...
        <div className="rounded-xl bg-red-50 border border-red-200 p-5" role="alert">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-[var(--color-outline-variant)] rounded-xl p-6 flex items-center justify-between shadow-sm">
          <div className="space-y-1">
            ...
            <div className="mt-2">
              <Badge variant={estadoToBadgeVariant(item.estado_actual)}>{estadoLabel(item.estado_actual)}</Badge>
            </div>
          ...
          <span
            className={`material-symbols-outlined text-3xl ${estadoIconColor(item.estado_actual)}`}
            aria-hidden="true"
          >
        ...
        <div className="bg-white border border-[var(--color-outline-variant)] rounded-xl p-6 flex items-center gap-6 shadow-sm">
          {similitud === 0 ? (
            <>
              <div
                className="w-16 h-16 flex items-center justify-center rounded-full bg-[var(--color-surface-container-high)] shrink-0"
                data-testid="similitud-pending"
              >
              ...
              <div className="flex-1">
                <h4 className="text-base font-semibold text-[var(--color-on-surface)]">Puntaje de Similitud</h4>
                <p className="text-xs text-[var(--color-secondary)] mt-1">Sin datos de similitud aún</p>
            ...
            <>
              ...
              <div className="flex-1">
                <h4 className="text-base font-semibold text-[var(--color-on-surface)]">Puntaje de Similitud</h4>
                <p className="text-xs text-[var(--color-secondary)] mt-1">{similitudMessage}</p>
              </div>
            </>
          )}
        </div>
        ...
        <div className="bg-white border border-[var(--color-outline-variant)] rounded-xl p-6 flex items-center gap-4 shadow-sm">
          <button
            type="button"
            className="flex-1 bg-[var(--color-primary)] text-white text-xs font-medium py-2 rounded-xl hover:opacity-90 transition-all active:scale-[0.98] shadow-sm"
          >
            Aceptar como Válido
          </button>
          <Link
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between px-1">
            ...
            <div className="flex gap-2">
              <button
                type="button"
                className="p-1 hover:bg-[var(--color-surface-container)] rounded-lg transition-colors"
        ...
        <div className="space-y-3">
          ...
          <div className="bg-white border border-[var(--color-outline-variant)] rounded-xl overflow-hidden shadow-sm">
            ...
            <div className="divide-y divide-slate-100">
              <div className="px-6 py-3 grid grid-cols-2 hover:bg-slate-50 transition-colors">
                ...
                <span className="text-sm font-bold text-[var(--color-on-surface)]">{item.fecha_deposito ?? "—"}</span>
              </div>
              <div className="px-6 py-3 grid grid-cols-2 hover:bg-slate-50 transition-colors">
              ...
              <div className="px-6 py-3 grid grid-cols-2 hover:bg-slate-50 transition-colors">
                ...
                <span className="text-sm font-bold text-[var(--color-on-surface)]">{item.banco ?? "—"}</span>
              </div>
          ...
          <div className="flex flex-col gap-2">
            <button
              type="button"
              className="w-full bg-[var(--color-error-container)] text-[var(--color-on-error-container)] text-xs font-medium py-4 rounded-xl hover:opacity-80 transition-all border border-[var(--color-error)]/20 flex items-center justify-center gap-2"
            >
